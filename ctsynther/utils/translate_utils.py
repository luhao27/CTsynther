...
from ctsynther.utils.smiles_utils import *
...
def translate_batch_original(model, batch, eos_idx=3, beam_size=10, max_length=200,
    ...
    src, tgt, gt_context_alignment, nonreactive_mask, graph_packs, _ = batch
    ...
    for step in range(0, max_length):
        ...
        for j in range(batch_size):
            ...
            if beam_size == eos_index.sum():  # all beam has finished
                ...
                origin_tokens = pred_tokens[j][torch.div(best_scores_id, num_words, rounding_mode="trunc")]
                ...
                if eos_index.sum() > 0:
                    ...
                    tmp_indices[:len(torch.div(best_scores_id, num_words, rounding_mode="trunc"))] = (torch.div(best_scores_id, num_words, rounding_mode="trunc"))
                    ...
                    selected_indices.append((torch.div(best_scores_id, num_words, rounding_mode="trunc")))
...
def translate_batch_search_strategy(model, batch, eos_idx=3, beam_size=10, max_length=200, invalid_token_indices=[], T=5,
